...
struct chd_interface_file *audit_hard_disk_open(const char *filename, const char *mode)
{
	...
	for (drv = hard_disk_gamedrv; drv != NULL; drv = drv->clone_of)
	{
		...
		if (file != NULL)
			return (struct chd_interface_file *)file;
	}
...
void audit_hard_disk_close(struct chd_interface_file *file)
{
...
UINT32 audit_hard_disk_read(struct chd_interface_file *file, UINT64 offset, UINT32 count, void *buffer)
{
...
UINT32 audit_hard_disk_write(struct chd_interface_file *file, UINT64 offset, UINT32 count, const void *buffer)
{
	return 0;
}
...
/*-------------------------------------------------
	audit_hard_disk_length - interface for sizing
	a hard disk image
-------------------------------------------------*/

UINT64 audit_hard_disk_length(struct chd_interface_file *file)
{
	UINT64 cur, len;

	cur = mame_ftell((mame_file *)file);
	mame_fseek((mame_file *)file, 0, SEEK_END);
	len = mame_ftell((mame_file *)file);
	mame_fseek((mame_file *)file, cur, SEEK_SET);
...
/* The vendored CHD library replaced the old hard_disk_* interface with this
   one, which also wants a length() callback. */
static struct chd_interface audit_hard_disk_interface =
{
	audit_hard_disk_open,
	audit_hard_disk_close,
	audit_hard_disk_read,
	audit_hard_disk_write,
	audit_hard_disk_length
};
...
int AuditRomSet (int game, tAuditRecord **audit)
{
	...
	/* BIOS alternatives, counted across the whole scan below. A driver like
	   neogeo declares fifteen of them (region/console/UniBIOS/debug); exactly
	   one is used, so reporting the other fourteen as missing ROMs buries the
	   file that actually matters under noise the player cannot act on. */
	bios_total = 0;
	bios_found = 0;
	...
	for (region = rom_first_region(gamedrv); region; region = rom_next_region(region))
		for (rom = rom_first_file(region); rom; rom = rom_next_file(rom))
		{
			if (ROMREGION_ISROMDATA(region))
			{
				...
				if (ROM_GETBIOSFLAGS(rom))
				{
					bios_total++;

					if (aud->status == AUD_ROM_GOOD)
						bios_found++;

					/* Drop the entry rather than listing it: an alternative
					   that is absent is not a fault. Whether the set has a
					   usable BIOS at all is decided once, after the scan. */
					if (aud->status == AUD_ROM_NOT_FOUND ||
						aud->status == AUD_NOT_AVAILABLE)
					{
						count--;
						continue;
			...
			else if (ROMREGION_ISDISKDATA(region))
			{
				...
				chd_set_interface(&audit_hard_disk_interface);
	...
	/* Skipping the alternatives above means a set with no BIOS at all would
	   otherwise audit clean and then fail inside the loader. Report it once,
	   naming the set to fetch rather than one arbitrary alternative. */
	if (bios_total > 0 && bios_found == 0 && count < AUD_MAX_ROMS)
	{
		const struct GameDriver *bios = gamedrv->clone_of;

		while (bios && !(bios->flags & NOT_A_DRIVER))
			bios = bios->clone_of;

		sprintf(aud->rom, "%s BIOS", bios ? bios->name : "system");
		aud->explength = 0;
		aud->length = 0;
		aud->exphash = NULL;
		hash_data_clear(aud->hash);
