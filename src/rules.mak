...
ifneq ($(CPU),)
OBJDIRS += $(OBJ)/cpu/adsp2100
# ADSP2105's boot-data loader reuses ADSP2104's (adsp2100.c:2107/2209) even
# though ADSP2104 isn't its own selectable CPU here - it must stay compiled in.
...
ifneq ($(SOUND),)
# pacman.c (and others) need SOUND_NAMCO_15XX/52XX/54XX from sndintrf.h,
# gated on their own HAS_ flags separate from plain HAS_NAMCO - not
# selectable SOUNDS entries here, just always compiled in alongside NAMCO.
...
# These 4 exist in sndintrf.h/src/sound but were never wired into rules.mak
# here (this project's rules.mak predates them being added upstream).

SOUND=$(strip $(findstring SCSP@,$(SOUNDS)))
ifneq ($(SOUND),)
SOUNDDEFS += -DHAS_SCSP=1
# scsplfo.c is #included from scsp.c itself, not its own translation unit.
...
# Hyperstone E1/E2 (vamphalf.c and others) - src/cpu/e132xs exists but was
# never wired into rules.mak here.
