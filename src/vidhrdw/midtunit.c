 ...
 *	Save state
 *
 *	local_videoram is the frame buffer the DMA blitter draws into and the
 *	screen is built from - a megabyte, and by far the largest thing in a T-unit
 *	state, but a state without it restores a game to a blank screen that then
 *	repaints only where something moves.
 *
 *	pen_map is not here on purpose: it is filled once at startup and never
 *	written again, so it is a constant lookup rather than state. dma_state is
 *	absent for the same kind of reason - it is scratch, set up and consumed
 *	inside a single blit, and never read across one.
 *
 *************************************/

static void midtunit_state_postload(void)
{
	/* The blit itself is synchronous - by the time the DMA busy bit matters,
	   the pixels are already in local_videoram, which we do save. What is
	   deferred is only the "done" signal, on a timer measured in microseconds,
	   and MAME's timers are not part of a save state. A state taken during
	   those microseconds would come back with the busy bit set and no timer
	   left to clear it, and the game would sit forever waiting on a DMA that
	   has already happened. Declaring it finished is true. */
	dma_register[DMA_COMMAND] &= ~0x8000;
}

static void midtunit_register_state(void)
{
	/* Registered here rather than at the top of VIDEO_START because
	   local_videoram is allocated a few lines above: registering a pointer
	   before it points anywhere is what left the TMS34010 shift register
	   filed as a null address. */
	state_save_register_UINT16("midtunit", 0, "local_videoram", local_videoram, 0x100000 / 2);

	state_save_register_UINT16("midtunit", 0, "control",        &midtunit_control, 1);
	state_save_register_UINT8 ("midtunit", 0, "videobank",      &videobank_select, 1);
	state_save_register_UINT32("midtunit", 0, "gfxbank_offset",  gfxbank_offset,
							   sizeof(gfxbank_offset) / sizeof(gfxbank_offset[0]));
	state_save_register_UINT16("midtunit", 0, "dma_register",    dma_register,
							   sizeof(dma_register) / sizeof(dma_register[0]));

	state_save_register_func_postload(midtunit_state_postload);
}
