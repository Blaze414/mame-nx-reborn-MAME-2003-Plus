...
/* namcos3d.h has no namcos22_lighting type - namcos22_camera has the same
   fields this code actually sets (power/ambient/x/y/z). */
static namcos22_camera mLighting;
		...
		INT32 flags )
{
	...
	/* namcos3d.h's namcos22_BlitTri takes one bias param, not separate
	   zoom+zcode - zcode (the z-sort/depth code) is the one that matches
	   what "bias" means elsewhere in this file; mWindowZoom (a window
	   scale factor, unrelated to per-tri depth) doesn't fit here and is
	   dropped rather than guess at a combined formula. */
	namcos22_BlitTri( pBitmap, &v[0], color, zcode, flags, &mLighting ); /* 0,1,2 */
	v[4] = v[0]; /* wrap */
	namcos22_BlitTri( pBitmap, &v[2], color, zcode, flags, &mLighting ); /* 2,3,0 */
} /* BlitQuadHelper */
