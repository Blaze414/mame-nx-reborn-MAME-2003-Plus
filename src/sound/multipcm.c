...
/* multi32.c wires this into its sound CPU's memory map, but nothing in
   the vendored tree implements it or documents the bank-offset encoding
   multi32's hardware writes here - no-op rather than guess (bank stays
   at its default, so playback just doesn't bank-switch samples). */
