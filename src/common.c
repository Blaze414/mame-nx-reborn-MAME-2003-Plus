...
/* libFLAC isn't vendored (see the f_type==2 branch below) - FLAC-compressed
   sample sets won't decode, everything else (WAV samples, ROMs, CHD) is
   unaffected. Define HAVE_FLAC once libFLAC is actually vendored. */
#ifdef HAVE_FLAC
...
int determine_bios_rom(const struct SystemBios *bios)
{
	...
	if(bios && (options.bios != NULL))
	{
		...
		if(options.bios == NULL || options.bios[0] == '\0')
