...
/* the "PSTARS" protection-chip emulation file these were meant to come
   from isn't in the vendored tree - no-op stubs so init_pstar() (Puzzle
   Star/Photo Y2K's protection init) links; that game's protection just
   won't be emulated, the rest of pgm.c is unaffected. */
READ16_HANDLER( PSTARS_protram_r ) { return 0; }
READ16_HANDLER( PSTARS_r16 ) { return 0; }
