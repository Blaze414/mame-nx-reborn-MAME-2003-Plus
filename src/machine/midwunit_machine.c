 ...
 *	Save state
 *
 *	Wolf-unit registered nothing, so a state carried the CPUs, RAM, palette and
 *	the T-unit video this hardware shares - which is why Ultimate Mortal Kombat
 *	3 saves, loads and looks correct - while none of the hardware in this file
 *	went with it. Most of what was missing is short-lived: cmos_write_enable is
 *	set and cleared two instructions apart, and the io and uart latches are
 *	rewritten by the game within a frame or two, so a state taken at any
 *	ordinary moment restores something the game immediately agrees with. That is
 *	why it appeared to work, which is not the same as it having been right - the
 *	difference only shows in the case nobody tests, a state taken while the
 *	security handshake is still mid-flight.
 *
 *	ioshuffle is included because WWF Mania rewrites it at runtime. For every
 *	other Wolf-unit game it is fixed after init, and saving it costs 16 bytes.
 *
 *************************************/

static void wunit_register_state(void)
{
	state_save_register_UINT8 ("midwunit", 0, "cmos_write_enable", &cmos_write_enable, 1);
	state_save_register_UINT8 ("midwunit", 0, "security_bits",     &security_bits, 1);
	state_save_register_UINT8 ("midwunit", 0, "analog_port",       &midxunit_analog_port, 1);
	state_save_register_UINT16("midwunit", 0, "iodata",             iodata,
							   sizeof(iodata) / sizeof(iodata[0]));
	state_save_register_UINT8 ("midwunit", 0, "ioshuffle",          ioshuffle, sizeof(ioshuffle));
	state_save_register_UINT8 ("midwunit", 0, "uart",               uart, sizeof(uart));
}
