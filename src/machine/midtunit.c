 ...
 *	Save state
 *
 *	The T-unit driver registered nothing at all, so a state carried the CPUs,
 *	main RAM, banks and palette - all of which the core registers on its own -
 *	and none of the hardware this file emulates. Restoring one left the
 *	protection chip mid-conversation with the game, which is not a subtle
 *	failure on Mortal Kombat II: mk2_prot_data is what the next protection read
 *	answers with.
 *
 *	Pointers are deliberately absent from the list. nbajam_prot_table,
 *	jdredd_prot_table and jdredd_hack point into ROM and into memory that is
 *	set up at init; registering an address is how the TMS34010 shift register
 *	came to be filed as null, and a table that never changes has nothing worth
 *	saving anyway.
 *
 *************************************/

static void tunit_state_postload(void)
{
	/* The DCS and ADPCM boards keep their own state - a sound CPU, its RAM and
	   the queues between it and the game - and none of it is registered, so a
	   loaded state would resume with the sound board still playing whatever it
	   was doing when the machine was saved, out of step with a game that has
	   moved somewhere else entirely. Resetting it is not free (music restarts
	   from the top) but it is the honest option: the board comes back in a
	   state the game agrees with, and everything else about the load is
	   correct. Same sequence MACHINE_INIT uses. */
	switch (sound_type)
	{
		case SOUND_ADPCM:
...
static void tunit_register_state(void)
{
	state_save_register_UINT8 ("midtunit", 0, "cmos_write_enable", &cmos_write_enable, 1);
	state_save_register_UINT8 ("midtunit", 0, "fake_sound_state",  &fake_sound_state, 1);

	state_save_register_UINT8 ("midtunit", 0, "mk_prot_index",     &mk_prot_index, 1);
	state_save_register_UINT16("midtunit", 0, "mk2_prot_data",     &mk2_prot_data, 1);

	state_save_register_UINT16("midtunit", 0, "nbajam_prot_queue",  nbajam_prot_queue,
							   sizeof(nbajam_prot_queue) / sizeof(nbajam_prot_queue[0]));
	state_save_register_UINT8 ("midtunit", 0, "nbajam_prot_index", &nbajam_prot_index, 1);

	state_save_register_UINT8 ("midtunit", 0, "jdredd_prot_index", &jdredd_prot_index, 1);
	state_save_register_UINT8 ("midtunit", 0, "jdredd_prot_max",   &jdredd_prot_max, 1);

	state_save_register_func_postload(tunit_state_postload);
}
...
static void init_tunit_generic(int sound)
{
	...
	/* after sound_type is set - the postload handler reads it to decide which
	   board to bring back */
	tunit_register_state();
}
