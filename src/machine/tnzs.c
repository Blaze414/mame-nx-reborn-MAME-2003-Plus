...
/* nothing in the vendored tree actually assigns this (likely dropped
   during some upstream refactor) - static buffer instead of NULL so a
   read/write here doesn't segfault, just misbehaves. */
static unsigned char tnzs_workram_storage[0x10000];
unsigned char *tnzs_workram = tnzs_workram_storage;
