 ...
 *	Save state
 *
 *	None of the three chips in this file registered anything, so every Midway
 *	game that uses one - the whole Wolf-unit line through the Seattle hardware -
 *	saved its CPUs and its RAM and left its security device behind.
 *
 *	It gets away with it nearly always, because the handshake these chips exist
 *	for runs once at boot and the device then sits idle at the end of its
 *	sequence. A state taken during play captures nothing in flight, which is why
 *	Ultimate Mortal Kombat 3 saved and loaded correctly with none of this. The
 *	failure needs a state taken inside the first seconds of the boot check, and
 *	then it is a security failure with no obvious cause.
 *
 *	Deliberately not registered: shuffle_map and irq_callback are pointers,
 *	default_nvram is configuration set before the machine runs, and has_dcs,
 *	has_cage, dcs_cpu and shuffle_type are all fixed by the driver at init.
 *	Registering an address rather than a value is how the TMS34010 shift
 *	register came to be filed as null.
 *
 *************************************/

static void serial_register_state(void)
{
	state_save_register_UINT8("midway_serial", 0, "data",    serial.data, sizeof(serial.data));
	state_save_register_UINT8("midway_serial", 0, "buffer", &serial.buffer, 1);
	state_save_register_UINT8("midway_serial", 0, "index",  &serial.index, 1);
	state_save_register_UINT8("midway_serial", 0, "status", &serial.status, 1);
	state_save_register_UINT8("midway_serial", 0, "bits",   &serial.bits, 1);
	state_save_register_UINT8("midway_serial", 0, "ormask", &serial.ormask, 1);
}
...
void midway_serial_pic2_init(int upper, int yearoffs)
{
	...
	/* The second-generation part carries the first one's serial state too. */
	serial_register_state();

	state_save_register_UINT16("midway_pic", 0, "latch",      &pic.latch, 1);
	state_save_register_UINT8 ("midway_pic", 0, "state",      &pic.state, 1);
	state_save_register_UINT8 ("midway_pic", 0, "index",      &pic.index, 1);
	state_save_register_UINT8 ("midway_pic", 0, "total",      &pic.total, 1);
	state_save_register_UINT8 ("midway_pic", 0, "nvram_addr", &pic.nvram_addr, 1);
	state_save_register_UINT8 ("midway_pic", 0, "buffer",      pic.buffer, sizeof(pic.buffer));
	/* The NVRAM has its own file through NVRAM_HANDLER, but it is also machine
	   state: a state restored without it would disagree with a bookkeeping page
	   the player had already changed. */
	state_save_register_UINT8 ("midway_pic", 0, "nvram",       pic.nvram, sizeof(pic.nvram));
}
...
void midway_ioasic_init(int shuffle, int upper, int yearoffs, void (*irq_callback)(int))
{
	...
	state_save_register_UINT32("midway_ioasic", 0, "reg",   ioasic.reg,
							   sizeof(ioasic.reg) / sizeof(ioasic.reg[0]));
	state_save_register_UINT8 ("midway_ioasic", 0, "shuffle_active", &ioasic.shuffle_active, 1);
	state_save_register_UINT8 ("midway_ioasic", 0, "irq_state",      &ioasic.irq_state, 1);
	state_save_register_UINT16("midway_ioasic", 0, "sound_irq",      &ioasic.sound_irq_state, 1);
	state_save_register_UINT8 ("midway_ioasic", 0, "auto_ack",       &ioasic.auto_ack, 1);
	state_save_register_UINT16("midway_ioasic", 0, "fifo",            ioasic.fifo,
							   sizeof(ioasic.fifo) / sizeof(ioasic.fifo[0]));
	state_save_register_UINT16("midway_ioasic", 0, "fifo_in",        &ioasic.fifo_in, 1);
	state_save_register_UINT16("midway_ioasic", 0, "fifo_out",       &ioasic.fifo_out, 1);
	state_save_register_UINT16("midway_ioasic", 0, "fifo_bytes",     &ioasic.fifo_bytes, 1);
	state_save_register_UINT32("midway_ioasic", 0, "fifo_empty_pc",
							   (UINT32 *)&ioasic.fifo_force_buffer_empty_pc, 1);
}
