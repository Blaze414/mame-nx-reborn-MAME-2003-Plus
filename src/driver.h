...
/* Was 0x0420, which is not a bit at all - it is GAME_IMPERFECT_SOUND |
   GAME_WRONG_COLORS, so "flags & GAME_DOESNT_SERIALIZE" was true for every
   driver with either of those. 314 driver entries declare imperfect sound and
   26 declare wrong colours; all of them reported themselves unable to save
   state, and state_get_dump_size() returned 0 for them, for reasons that have
   nothing to do with serialization. 0x0800 is the next free bit in this
   field. */
#define GAME_DOESNT_SERIALIZE       0x0800	/* game can not be saved through serailization */
