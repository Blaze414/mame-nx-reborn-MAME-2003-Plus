...
/* osd_fopen() (fileio.h) returns a plain FILE* now - unzip.c still calls
   these opaque-handle-style wrappers from the older osdepend.h contract,
   so they're just thin forwards to the standard C library (implemented in
   nx_fileio.c). */
