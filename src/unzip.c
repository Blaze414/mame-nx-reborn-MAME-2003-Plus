...
*/
/* ASCII-only uppercase fold - zip entry names are ASCII, and locale-aware
   toupper() needs newlib's __locale_ctype_ptr, which this toolchain's libc
   build doesn't link in. */
#define ASCII_UPPER(c) (((c) >= 'a' && (c) <= 'z') ? (c) - 'a' + 'A' : (c))
...
static int equal_filename(const char* zipfile, const char* file) {
	...
	if (s2)
		...
		s2 = zipfile;
	while (*s1 && ASCII_UPPER(*s1)==ASCII_UPPER(*s2)) {
		++s1;
		++s2;
	}
	return !*s1 && !*s2;
