...
/* Staging buffer for the shift register.

   state.shiftreg is a pointer allocated in tms34010_reset(), which runs after
   tms34010_init() - so registering the pointer itself, as this used to, filed a
   null address in the save registry and left it there permanently. Everything
   downstream trusts that address: state_get_dump_size() bails at the first
   entry with no data and returns zero, which is reported as "this machine
   cannot save its state", and every TMS34010 game in the set - the whole Midway
   Y/T/Wolf unit line, so Mortal Kombat, NBA Jam, Smash TV, Terminator 2 - was
   unsaveable for that reason alone.

   A staging buffer rather than moving the allocation earlier: the registry
   stores one address per entry, while state.shiftreg is inside the block that
   the context switch copies, so each CPU carries its own pointer and a single
   captured address could only ever be right for one of them. The presave and
   postload hooks below run with the correct context already pushed, so copying
   through a buffer at a fixed address is right for all of them. */
static UINT16 shiftreg_save[SHIFTREG_SIZE / sizeof(UINT16)];
...
void tms34010_init(void)
{
	...
	state_save_register_UINT16("tms34010", cpunum, "SHIFTREG",  shiftreg_save, SHIFTREG_SIZE / sizeof(UINT16));
...
void tms34010_reset(void *param)
{
	...
	/* Carried across the zap below. This used to malloc a fresh one on every
	   reset and drop the old pointer on the floor - 8KB per reset, and the
	   pause menu has a Reset Game row. */
	data16_t *shiftreg = state.shiftreg;
	...
	if (!shiftreg)
		shiftreg = malloc(SHIFTREG_SIZE);
	state.shiftreg = shiftreg;
	if (shiftreg)
		memset(shiftreg, 0, SHIFTREG_SIZE);
...
void tms34010_state_save(int cpunum, mame_file *f)
{
	...
	mame_fwrite(f,state.shiftreg,SHIFTREG_SIZE);
}
...
void tms34010_state_load(int cpunum, mame_file *f)
{
	...
	mame_fread(f,state.shiftreg,SHIFTREG_SIZE);
}
...
static void tms34010_state_presave(void)
{
	...
	/* Into the fixed address the registry holds - see shiftreg_save. */
	if (state.shiftreg)
		memcpy(shiftreg_save, state.shiftreg, SHIFTREG_SIZE);
}
...
static void tms34010_state_postload(void)
{
	...
	if (state.shiftreg)
		memcpy(state.shiftreg, shiftreg_save, SHIFTREG_SIZE);
